...
RnsShell::LayerInvalidateMask  RSkComponentRNSVGUse::updateComponentProps(const ShadowView &newShadowView,bool forceUpdate) {
  ...
  setLengthAttribute(selfNode,SkSVGAttribute::kX,newRNSVGUseProps.x.c_str());
  setLengthAttribute(selfNode,SkSVGAttribute::kY,newRNSVGUseProps.y.c_str());
  setStringAttribute(selfNode,SkSVGAttribute::kHref,newRNSVGUseProps.href.c_str());
  setLengthAttribute(selfNode,SkSVGAttribute::kWidth,newRNSVGUseProps.width.c_str());
  setLengthAttribute(selfNode,SkSVGAttribute::kHeight,newRNSVGUseProps.height.c_str());
  
  updateCommonNodeProps(newRNSVGUseProps,*this,selfNode);
...
void RSkComponentRNSVGUse::appendChild(sk_sp<SkSVGNode>) {
    RNS_LOG_INFO("cannot append child nodes to an SVG shape.\n");
}
